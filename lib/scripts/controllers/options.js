...
import {
  ...
  QuizzingOral,
  QuizzingAural,
} from '../services/quizzing';
import { ProgressActivity } from '../utils/progressActivity';
import { ContentScript } from '../mtw';
...
export class OptionCtrl {
  ...
  constructor($scope, $timeout) {
    ...
    this.activityChartLoaderVisible = true;
    this.appLoader = true;
    this.tsExampleLoader = true;
    this.initiateActivityProgress();
  }

  /**
   * Initialize Activity Progress chart along with D3.js functions
   */
  ...
  getData() {
    chrome.storage.local.get(null, data => {
      ...
      this.blacklistedWords = data.userBlacklistedWords
        .slice(1, -1)
        .split('|')
        .filter(function(n) {
          return n !== '';
        });
      this.blacklistedWebsites = data.blacklist
        .slice(1, -1)
        .split('|')
        .filter(function(n) {
          return n !== '';
      ...
      this.userDefinedTranslationList = this.toList(
        this.userDefinedTranslations
      );
      ...
      this.randomTranslatedWordsForQuiz = this.randomlySelectTenWords(
        this.translatedWordsForQuiz
      );
      this.translatedWordsExist =
        Object.keys(this.randomTranslatedWordsForQuiz).length === 0
          ? false
          : true;
      this.quizOptions = this.getShuffledValues(
        this.randomTranslatedWordsForQuiz
      );
      this.date = new Date();
  ...
  getYandexLimit(period) {
    if (this.currentMonth in this.stats['translatorWiseWordCount'][0]) {
      var limit;
      switch (period) {
        case 'monthly':
          limit =
            (this.stats['translatorWiseWordCount'][0][this.currentMonth][
              'Yandex'
            ][2] /
              10000000) *
            100;
        ...
        case 'daily':
          limit =
            (this.stats['translatorWiseWordCount'][1][this.currentDay][
              'Yandex'
            ][2] /
              1000000) *
            100;
      ...
      if (limit > 100) {
        return 100.0;
      } else return parseFloat(limit).toFixed(2);
    }
  }
  ...
  activatePattern(index) {
    if (this.hasKey(this.patterns[index][4])) {
      this.status(
        'You do not have a key to for the translator',
        2000,
        100,
        'danger'
      );
  ...
  createPattern() {
    if (this.srcLang === undefined || this.targetLang === undefined) {
      ...
      for (var index in patterns) {
        if (
          patterns[index][0][0] === src[1] &&
          patterns[index][1][0] === trg[1] &&
          patterns[index][2] === prb &&
          patterns[index][4] === translator
        ) {
          duplicateInput = true;
  ...
  changeApiKey(translator) {
    ...
    switch (translator) {
      case 'Yandex':
        if (
          /^\s*$/.test(this.yandexTranslatorApiKey) &&
          this.translatorService === 'Yandex'
        ) {
          this.deactivatePatterns();
          this.toggleDoNotTranslate();
        }
        if (
          this.currentMonth in stats['translatorWiseWordCount'][0] &&
          'Yandex' in stats['translatorWiseWordCount'][0][this.currentMonth]
        ) {
          stats['translatorWiseWordCount'][0][this.currentMonth][
            'Yandex'
          ][2] = 0;
      ...
      case 'Google':
        if (
          /^\s*$/.test(this.googleTranslatorApiKey) &&
          this.translatorService === 'Google'
        ) {
          this.deactivatePatterns();
          this.toggleDoNotTranslate();
        }
        if (
          'Google' in stats['translatorWiseWordCount'][0][this.currentMonth]
        ) {
          stats['translatorWiseWordCount'][0][this.currentMonth][
            'Google'
          ][2] = 0;
      ...
      case 'Azure':
        if (
          /^\s*$/.test(this.azureTranslatorApiKey) &&
          this.translatorService === 'Azure'
        ) {
          this.deactivatePatterns();
          this.toggleDoNotTranslate();
        }
  ...
  addBlackListedWord() {
    if (
      /^\s*$/.test(this.newBlacklistWord) === false &&
      this.blacklistedWords.indexOf(this.newBlacklistWord) === -1
    ) {
      this.blacklistedWords.push(this.newBlacklistWord.trim());
      this.newBlacklistWord = '';
      this.blacklistWordPage =
        this.range(this.blacklistedWords.length).length - 1;
      this.$timeout(() => {
        this.$scope.$apply();
      });
      this.save(
  ...
  addBlackListedWebsite() {
    if (
      /^\s*$/.test(this.newBlacklistWebsite) === false &&
      this.blacklistedWebsites.indexOf(this.newBlacklistWebsite) === -1
    ) {
      this.blacklistedWebsites.push(this.newBlacklistWebsite.trim());
      this.newBlacklistWebsite = '';
      this.blacklistWebsitePage =
        this.range(this.blacklistedWebsites.length).length - 1; //set to last page
      this.$timeout(() => {
        this.$scope.$apply();
      });
      this.save(
  ...
  removeBlackListedWebsite(website) {
    this.blacklistedWebsites.splice(
      this.blacklistedWebsites.indexOf(website),
      1
    );
  ...
  addUserDefinedTranslation() {
    if (
      /^\s*$/.test(this.original) === false &&
      /^\s*$/.test(this.translated) === false
    ) {
      this.userDefinedTranslations[this.original] = this.translated;
      this.userDefinedTranslationList = this.toList(
        this.userDefinedTranslations
      );
      this.userDefinedTranslationPage =
        this.range(this.userDefinedTranslationList.length).length - 1;
      this.original = '';
      this.translated = '';
      this.$timeout(() => {
        this.$scope.$apply();
  ...
  saveTranslation() {
    if (
      ...
      this.savedTranslationList = this.toList(this.savedTranslations);
      this.savedTranslationPage =
        this.range(this.savedTranslationList.length).length - 1;
      this.original = '';
      this.translated = '';
      this.$timeout(() => {
        this.$scope.$apply();
  ...
  addLearntWord() {
    if (
      /^\s*$/.test(this.newLearntWord) === false &&
      this.learntWords.indexOf(this.newLearntWord) === -1
    ) {
  ...
  validateKeysFile(data) {
    if (
      'googleTranslatorApiKey' in data &&
      'azureTranslatorApiKey' in data &&
      'yandexTranslatorApiKey' in data
    ) {
      return true;
    } else {
      return false;
  ...
  scanCheckbox() {
    var statusAllSettings = document.getElementById('backup-allSettings')
        .checked,
    ...
    if (
      ...
      // when none of the checkbox is checked
    ...
    fileInput.addEventListener('change', e => {
      ...
      if (file.type.match(textType)) {
        ...
        reader.onload = e => {
          ...
          if (
            this.validateKeysFile(data.keys) &&
            this.validateLearntWords(data) &&
            this.validateSavedTrans(data)
          ) {
            if (
              data.keys.azureTranslatorApiKey.length ||
              data.keys.yandexTranslatorApiKey.length ||
              data.keys.googleTranslatorApiKey.length
            ) {
            ...
            if (data.learnt.length > 2) {
              // default null case is "()"
              this.learntWords = data.learnt;
              this.save(
                { learntWords: data.learnt },
                'Restored Learnt Words from file'
              );
              this.$timeout(() => {
                this.$scope.$apply();
              });
            ...
            if (data.saved.length) {
              ...
              this.save(
                { savedTranslations: data.saved },
                'Restored Saved Translations from file'
              );
              this.$timeout(() => {
                this.$scope.$apply();
              });
  ...
  checkAnswer(index) {
    ...
    if (
      ...
      this.randomTranslatedWordsForQuiz[words[index]]
    ) {
  ...
  /**
   * Generate graphs based on the usage statistics.
   */
  generateStats() {
    var wordsTranslatedArea = document
      .getElementById('wordsTranslatedArea')
      .getContext('2d');
    var wordsTransPatterns = document
      .getElementById('wordsTransPatterns')
      .getContext('2d');
    ...
    chrome.storage.local.get(['nOfWordsTransEveryDay'], function(result) {
      if (!result.nOfWordsTransEveryDay) return;
  ...
  /**
   * Check if a quiz would execute perfectly in the browser.
   */
  quizPrimCheck() {
    let temp;

    // aural quiz
    // disabled for UI improvements
  ...
  /**
   * Initialize quizzing section and activate browser checks for finding the supported quizzes.
   */
  initialiseQuizWrapper() {
    ...
    if (
      ...
      this.$scope.showWarningOral = true;
    }
  }

  /**
   * Activate the quiz handler class bsed on the selection of the type of quiz.
   * @param {String} stats - code of the quiz type
   */
  updateQuizSection(stats) {
    switch (stats) {
      ...
      case 'quizOral':
        document.getElementById('messageUserSpeechID').style.backgroundColor =
          '#fff';
        this.$scope.quizSection = false;
        this.$scope.quizOralTranslation = true;
        this.QuizzingOral.initMessageDiv();
        this.QuizzingOral.setQuestions();
        break;

      case 'quizAural':
        this.$scope.quizSection = false;
        this.$scope.quizAuralTranslation = true;
        this.QuizzingAural.setQuestionWords();
        break;
    }
  }
  ...
  /**
   * Controls "listening" label in oral quiz.
   * @param {number} val - constant increment of counter
   */
  ...
  /**
   * Validates and controls the background color of oral quiz based on the result.
   * @param {String} code - code of quiz type
   */
  quizValidateAnswer(code) {
    switch (code) {
      case 'quiz-oral-done':
        this.$scope.quizOralResult = this.QuizzingOral.checkAnswer(
          document.getElementById('messageUserSpeechID').value
        );
        if (this.$scope.quizOralResult) {
          document.getElementById('messageUserSpeechID').style.backgroundColor =
            '#E8F5E9';
        } else {
          document.getElementById('messageUserSpeechID').style.backgroundColor =
  ...
  /**
   * Controls playing of audio in aural quiz and sets the target language of the output.
   */
  speak() {
    let t = document
      .getElementById('quiz-aural-question')
  ...
  /**
   * Initialize HTML snippet for translation settings.
   */
  setTSExample() {
    let exampleElement = document.getElementById('ts-example-text'),
      example = exampleElement.innerText;
    if (this.srcLang !== 'en' && this.srcLang !== '') {
      chrome.storage.local.get(null, data => {
        ...
        translatorObject.translateSentence(example).then(tMap => {
          exampleElement.innerHTML = tMap;
          this.translationSettingsEg();
        });
      });
    } else {
  ...
  /**
   * Translate the sample HTML snippet for translation settings example.
   */
  translationSettingsEg() {
    let parentElementID = 'ts-example',
      parentElement = document.getElementById(parentElementID),
      mtwTranslatorInstance = new ContentScript();
    chrome.storage.local.get(null, res => {
      mtwTranslatorInstance.initialize(res, true);
      mtwTranslatorInstance.injectCSS(res.translatedWordStyle);
      this.tsExampleLoader = false;
      mtwTranslatorInstance.translate();
    });
  }
}

angular
  .module('MTWOptions', ['lazy-scroll'])
