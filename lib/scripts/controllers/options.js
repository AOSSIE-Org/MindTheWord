...
export class OptionCtrl {
  ...
  constructor($scope, $timeout) {
    ...
    //this.nOfWordsTransEveryDay = [];
    //this.save({nOfWordsTransEveryDay: []});
  ...
  backupSavedTranslations() {
    console.warn('here');
    chrome.storage.local.get('savedTranslations', (data) => {
      console.warn(data);
      saveFile(data, 'mtw_savedTranslations.txt');
    });
  }

  backupLearntWords() {
    chrome.storage.local.get('learntWords', (data) => {
      saveFile(data, 'mtw_learntWords.txt');
    });
  }
  ...
  restoreSavedTranslations() {
    let fileInput = document.getElementById('savedTrans-file');
    ...
    fileInput.addEventListener('change', (e) => {
      ...
      if (file.type.match(textType)) {
        ...
        reader.onload = (e) => {
          if (this.validateSavedTrans(reader.result)) {
            var saved = JSON.parse(reader.result);
            this.savedTranslations = saved.savedTranslations;
            this.save(saved, 'Restored Saved Translations from file');
            this.$timeout(() => {
              this.$scope.$apply();
            });
          } else {
  ...
  restoreLearntWords() {
    let fileInput = document.getElementById('learnt-file');
    ...
    fileInput.addEventListener('change', (e) => {
      ...
      if (file.type.match(textType)) {
        let reader = new FileReader();
        reader.onload = (e) => {
          if (this.validateLearntWords(reader.result)) {
            console.warn('inside');
            var learn = JSON.parse(reader.result);
            this.learntWords = learn.learntWords;
            this.save(learn, 'Restored Learnt Words from file');
            this.$timeout(() => {
              this.$scope.$apply();
            });
          } else {
  ...
  validateSavedTrans(data) {
    try {
      ...
      if (!('savedTranslations' in inJSON)) {
        throw new Error('in valid savedTransations file');
      }
    } catch(e) { // catches invalid JSON format and wrong file selection
      return false;
    }
    return true;
  }
  ...
  validateLearntWords(data) {
    try {
      let inJSON = JSON.parse(data);
      console.warn(inJSON);
      if (!('learntWords' in inJSON)) {
        throw new Error('in valid learntWords file');
      }
    } catch(e) { // catches invalid JSON format and wrong file selection
      return false;
    }
      ...
      if (file.type.match(textType)) {
        ...
        reader.onload = (e) => {
          ...
          if (this.validateKeysFile(keys)) {
            ...
            this.azureTranslatorApiKey = keys.azureTranslatorApiKey;
