...
export class OptionCtrl {
  ...
  changeApiKey(translator) {
    ...
    switch (translator) {
      case 'Yandex':
        ...
        if (this.currentMonth in stats['translatorWiseWordCount'][0] && 'Yandex' in stats['translatorWiseWordCount'][0][this.currentMonth]) {
      ...
      case 'Google':
        ...
        if ('Google' in stats['translatorWiseWordCount'][0][this.currentMonth]) {
  ...
  generateStats() {
    ...
    chrome.storage.local.get(['nOfWordsTransEveryDay'], function(result) {
      if(!result.nOfWordsTransEveryDay) 
        return;
  ...
  updateQuizSection(stats) {
    switch(stats) {
      ...
      case 'quizOral':
        document.getElementById('messageUserSpeechID').style.backgroundColor = '#fff';
        this.$scope.quizSection = false;
    ...
    switch(code) {
      case 'quiz-oral-done':
        ...
        if (this.$scope.quizOralResult) {
          document.getElementById('messageUserSpeechID').style.backgroundColor = '#E8F5E9';
        } else {
          document.getElementById('messageUserSpeechID').style.backgroundColor = '#FFEBEE';
        }
  ...
  setTSExample() {
    ...
    if (this.srcLang !== 'en' && this.srcLang !== '') {
      chrome.storage.local.get(null, data => {
        ...
        switch (this.translatorService) {
          case 'Yandex':
            translatorObject = new YandexTranslate(this.yandexTranslatorApiKey, 'en', this.srcLang);
            break;
          case 'Azure':
            translatorObject = new AzureTranslate(this.azureTranslatorApiKey, 'en', this.srcLang);
            break;
          case 'Google':
            translatorObject = new GoogleTranslate(this.googleTranslatorApiKey, 'en', this.srcLang);
            break;
          case null:
            console.log('No translator selected');
          default:
