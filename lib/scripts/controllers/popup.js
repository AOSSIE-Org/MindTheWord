  ...
  constructor($scope, $timeout, $window) {
    ...
    chrome.storage.local.get(
      ...
      data => {
        ...
        this.createStatNode(
          'Words Translated : ',
          this.stats.totalWordsTranslated
        );
        this.createStatNode('Learnt Words : ', this.learntWords.length);
        this.createStatNode(
          'Blacklisted Words : ',
          this.blacklistedWords.length
        );
        this.createStatNode(
          'Blacklisted Websites : ',
          this.blacklistedWebsites.length
        );
        chrome.tabs.query({ active: true, currentWindow: true }, tabs => {
          this.isBlacklisted = this.blacklistWebsiteReg.test(tabs[0].url);
          this.tabURL = tabs[0].url;
          if (this.isBlacklisted) {
            document.getElementById('mtwPopBlacklistImage').src =
              '../assets/img/whitelistWebsite.png';
          }
        });
...
angular.module('MTWPopup', []).controller('PopupCtrl', PopupCtrl);
