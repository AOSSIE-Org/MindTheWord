        ...
        for (let i = 0; i < translatedWords.length; i++) {
          ...
          for(var key in this.countedWords){
            if(this.countedWords.hasOwnProperty(key)){
              if(key === word){
                hoverHTML += '<ul><li>Occurs '+ this.countedWords[key] +' time(s) in current page </li>';
                break;
              }
            }
          }
          for(var k =0; k< nativeWords.length; k++) {
            if (nativeWords[k].word===translatedWords[i].getAttribute('data-translated')){
              ...
              (Math.round(( nativeWords[k].count / this.totalWordsLanguage ) * 100) == 0 
                ?  ((Math.round(( nativeWords[k].count / this.totalWordsLanguage )*100) * 1000 == 0) ? ' rarely </li></ul>' :
                ' approx. '+Math.round(( nativeWords[k].count / this.totalWordsLanguage )*10) * 1000+' time(s) in every 1000 words </li></ul>')
                :' approx. '+ Math.round(( nativeWords[k].count / this.totalWordsLanguage ) * 100)+' time(s) in every 100 words </li></ul>'); // prevent showing 0 in cases of ratios of < 0.5
              break;
            }
          }
