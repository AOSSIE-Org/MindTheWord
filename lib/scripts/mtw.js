...
export class ContentScript {
  ...
  filterDoms() {
    ...
    for (let i = loopStart; i < domLength; i++) {
  ...
  initialize(res, isTSExample) {
    ...
    this.translatedWordsForQuiz = JSON.parse(res.translatedWordsForQuiz);
    this.activityProgressStore = res.activityProgressStore;
    ...
    if (!isTSExample) {
      ...
      let ele = document.getElementById('ts-example');
      this.allparagraphs = document
        .getElementById('ts-example')
        .querySelectorAll('p');
    }
    ...
    this.custom_icon.push(
      chrome.extension.getURL('assets/img/occasionally.png')
    );
  ...
  translate() {
    ...
    if (this.userDefinedOnly === true) {
      filteredWords = this.filterToUserDefined(
        countedWords,
  ...
  getTranslator() {
    ...
    switch (this.translator) {
      case 'Yandex':
        translatorObject = new YandexTranslate(
        ...
        break;
      case 'Azure':
        translatorObject = new AzureTranslate(
        ...
        break;
      case 'Google':
        translatorObject = new GoogleTranslate(
  ...
  getTranslatorInv(src, target) {
    ...
    switch (this.translator) {
      case 'Yandex':
        ...
        break;
      case 'Bing':
        translatorObject = new BingTranslate(
  ...
  getCommonWords() {
    var commonWordsURI = chrome.extension.getURL(
      './common/' + this.srcLang + '.json'
    );
  ...
  getAllWords(ngramMin, ngramMax) {
    ...
    for (var i = 0; i < this.allparagraphsFiltered.length; i++) {
      if (
        this.isInViewport(this.allparagraphsFiltered[i]) &&
        this.check_array[i] &&
  ...
  filterParagraphsQuizzing() {
    chrome.storage.local.get('quizQuestionsStore', res => {
      res = res.quizQuestionsStore;
      let quizSample = document.querySelectorAll('p');
      quizSample.forEach(domNode => {
        let domSize = domNode.innerText.trim().length;
        if (domSize > 100 && domSize < 200) {
          let inst = {
            type: 'sentence',
            text: domNode.innerText.trim(),
            shown: 0,
            atempted: 0,
            correct: 0,
            score: 0,
            length: domNode.innerText.trim().length,
          };
          let len = Object.keys(res).length;
          len++;
          res['quiz-question-' + len.toString()] = inst;
          chrome.storage.local.set({ quizQuestionsStore: res });
        } else if (domSize >= 200 && domSize < 500) {
          // skip large size paragraphs
          let inst = {
            type: 'paragraph',
            text: domNode.innerText.trim(),
            shown: 0,
  ...
  filter(
    ...
    var countedWordsList = this.shuffle(
      this.toList(countedWords, (word, count) => {
        if (this.clkTest(word))
          return (
            !!word &&
            word !== '' &&
            !/\d/.test(word) && // no empty words
            !blackListReg.test(word.toLowerCase()) && // no blacklisted words
            !punctuationReg.test(word.toLowerCase())
          );
        // no punctuation marksreturn !!word && word.length >= 2 && // no words that are too short
        else
          return (
            word !== '' &&
            !/\d/.test(word) && // no empty words
    ...
    var targetLength = Math.floor(
      (Object.keys(countedWordsList).length * translationProbability) / 100
    );
  ...
  activityProgress(translations) {
    ...
    let prev = this.activityProgressStore[date],
      URL = window.location.href,
      LimitDays = 30;
    LimitDays -= 1;
    if (prev === undefined) {
      this.activityProgressStore[date] = {
        'websites-visited': [],
        'words-learnt': 0,
        'saved-translation': 0,
        day: new Date().getDay(),
      ...
      let max = 0;
      for (let keys in this.activityProgressStore) {
        max =
          this.activityProgressStore[keys]['day-number'] > max
            ? this.activityProgressStore[keys]['day-number']
            : max;
      }
      this.activityProgressStore[date]['day-number'] = max + 1;
      console.log('max is ' + max);
      // delete if records found beyond 30 days
      if (max > LimitDays) {
        console.log('inside 1');
        let incr = max - LimitDays;
        for (let keys in this.activityProgressStore) {
          if (this.activityProgressStore[keys]['day-number'] <= incr) {
            console.log('inside 2');
            console.log(this.activityProgressStore[keys]);
            delete this.activityProgressStore[keys];
          }
        }
        // re-arrange day-number
        for (let keys in this.activityProgressStore) {
          this.activityProgressStore[keys]['day-number'] -= incr;
        }
      }
    }
    // add websites to the list
    let visited = this.activityProgressStore[date]['websites-visited'],
      found = false;
    for (let web in visited) {
      if (visited[web]['url'] === URL) {
        found = true;
        break;
      }
    }
    if (!found) {
      this.activityProgressStore[date]['websites-visited'].push({
        url: URL,
        tMap: {},
        'translation-count': 0,
      });
    ...
    for (let webObjects in this.activityProgressStore[date][
      'websites-visited'
    ]) {
      if (
        this.activityProgressStore[date]['websites-visited'][webObjects][
          'url'
        ] === URL
      ) {
        for (let keys in translations) {
          this.activityProgressStore[date]['websites-visited'][webObjects][
            'tMap'
    ...
    chrome.storage.local.set({
      activityProgressStore: this.activityProgressStore,
    });
  }
  ...
  processTranslations(translationMap, userDefinedTMap, cwMatch) {
    ...
    for (var w in translationMap) {
      if (
        ...
        translationMap[w] !== '' &&
        !userDefinedTMap[w] &&
        !this.containsIllegalCharacters(translationMap[w])
      ) {
    ...
    for (w in cwMatch) {
      if (
        ...
        cwMatch[w] !== '' &&
        !userDefinedTMap[w] &&
        !this.containsIllegalCharacters(cwMatch[w])
      ) {
    ...
    this.activityProgress(filteredTMap);
    ...
    for (let a in filteredTMap) {
      this.translatedWordsForQuiz[a] = filteredTMap[a];
    }
    ...
    //for quiz feature
    chrome.storage.local.set({
      translatedWordsForQuiz: JSON.stringify(this.translatedWordsForQuiz),
    });
    ...
    if (!this.userDefinedOnly) {
      this.stats['translatorWiseWordCount'][0][currentMonth][
        this.translator
      ][0] += numberOfTranslatedWords;
      this.stats['translatorWiseWordCount'][0][currentMonth][
        this.translator
      ][1] += numberOfTranslatedCharacters;
      this.stats['translatorWiseWordCount'][0][currentMonth][
        this.translator
      ][2] += numberOfTranslatedCharacters;
      this.stats['translatorWiseWordCount'][1][currentDay][
        this.translator
      ][0] += numberOfTranslatedWords;
      this.stats['translatorWiseWordCount'][1][currentDay][
      ...
      chrome.storage.local.set({
        numberOfTranslatedWords: JSON.stringify(
          this.numberOfTranslatedWordsOnPage
        ),
      });
    }
    ...
    if (Object.keys(filteredTMap).length !== 0) {
      ...
      if (this.oneWordTranslation) {
        for (var i = 0; i < paragraphs.length; i++) {
          this.translateOneWord(
            paragraphs[i],
            filteredTMap,
            this.invertMap(filteredTMap)
          );
        }
      } else {
        for (var i = 0; i < paragraphs.length; i++) {
    ...
    var wordFrequencyPromise = new Promise((resolve, reject) => {
      var allTranslated = document.querySelectorAll(
          '.mtwTranslatedWord, .mtwTranslatedWorde, .mtwTranslatedWordn, .mtwTranslatedWordh'
        ),
        nativeWords = [], // storing language specific words
        totalWordsLanguage; // language specific counter for calculating rarity ratio
      chrome.storage.local.get(
        ['wordFrequencyCorpus', 'totalWordsperLanguage'],
        wordObjectList => {
          ...
          for (var x = 0; x < totalWordsLanguage.length; x++) {
            if (totalWordsLanguage[x].targetLang === this.targetLanguage) {
              ...
              totalWordsLanguage[x].targetLang !== this.targetLanguage &&
              x === totalWordsLanguage.length - 1
            ) {
              totalWordsLanguage.push({
                targetLang: this.targetLanguage,
                count: allTranslated.length,
          ...
          chrome.storage.local.set({
            totalWordsperLanguage: totalWordsLanguage,
          });
          resolve(nativeWords);
    ...
    wordFrequencyPromise
      .then(nativeWords => {
        // Adding buttons and frequency counters to Hover-Card
        chrome.storage.local.get(
          ['learntWords', 'translatedWordsForQuiz'],
          wordsListObj => {
            ...
            for (let a = 0; a < paragraphs.length; a++) {
              var translatedWords = paragraphs[a].querySelectorAll(
                '.mtwTranslatedWord, .mtwTranslatedWorde, .mtwTranslatedWordn, .mtwTranslatedWordh'
              );
              for (let i = 0; i < translatedWords.length; i++) {
                ...
                if (!wordsArray.includes(word)) {
                  ...
                  var hoverHTML =
                    '<p>' +
                    translatedWords[i].getAttribute('data-original') +
                    '</p>';
                  for (var k = 0; k < nativeWords.length; k++) {
                    if (
                      nativeWords[k].word ===
                      translatedWords[i].getAttribute('data-translated')
                    ) {
                      if (
                        Math.round(
                          (nativeWords[k].count / this.totalWordsLanguage) *
                            1000
                        ) <= 1
                      ) {
                        ...
                        Math.round(
                          (nativeWords[k].count / this.totalWordsLanguage) * 100
                        ) >= 4
                      ) {
                        hoverHTML +=
                          '<img src="' +
                        ...
                        hoverHTML +=
                          '<img src="' +
                          this.custom_icon[7] +
                          '" class="tags-occasional">';
                        hoverBackground = '#FFFDE7';
                      }
                    }
                  }
                  hoverHTML +=
                    '<img style="cursor:pointer;" class="mtwSpeak" title="Speak" data-translated="' +
                    translatedWords[i].getAttribute('data-translated') +
                    '" src="' +
                    this.custom_icon[0] +
                    '">';
                  if (this.detailsApiKey && this.optionalAPI) {
                    hoverHTML +=
                      '<img style="cursor:pointer;" class="moreInformation" title="More Information" data-original="' +
                      translatedWords[i].getAttribute('data-original') +
                      '" src="' +
                      this.custom_icon[11] +
                      '">';
                  ...
                  hoverHTML +=
                    '<img style="cursor:pointer;" class="mtwMarkAsLearnt mtwHovercardBelow" title="Mark As Learnt" data-original="' +
                    translatedWords[i].getAttribute('data-original') +
                    '" src="' +
                    this.custom_icon[1] +
                    '">';
                  hoverHTML +=
                    '<img style="cursor:pointer;" class="mtwSaveTranslation mtwHovercardBelow" title="Save Translation" data-translated="' +
                    translatedWords[i].getAttribute('data-translated') +
                    '" data-original="' +
                    translatedWords[i].getAttribute('data-original') +
                    '" src="' +
                    ...
                    '">';
                  hoverHTML +=
                    '<img style="cursor:pointer;" class="mtwBlacklistWord mtwHovercardBelow" title="Blacklist Word" data-original="' +
                    translatedWords[i].getAttribute('data-original') +
                    '" src="' +
                    this.custom_icon[5] +
                    '">';
                  hoverHTML +=
                    '<img style="cursor:pointer;" class="mtwWordInfo mtwHovercardBelow" title="Word Info" data-translated="' +
                    translatedWords[i].getAttribute('data-translated') +
                    '" src="' +
                    this.custom_icon[3] +
                    '">';
                  hoverHTML +=
                    '<img style="cursor:pointer;" class="mtwVisualHint mtwHovercardBelow" title="Visual Hint" data-translated="' +
                    translatedWords[i].getAttribute('data-translated') +
                    '" src="' +
                    this.custom_icon[2] +
                    '">';
                  ...
                  let word = translatedWords[i].getAttribute('data-translated'),
                    wordNativeLang = translatedWords[i].getAttribute(
                      'data-original'
                    ),
                    wordsLearnt = JSON.parse(
                      wordsListObj.translatedWordsForQuiz
                    );
                  ...
                  var hoverHTML =
                    '<p>Choose <strong>correct</strong> Translations:</p><div>';
                  for (let j = 0; j < options.length; j++) {
                    if (j === correctAnsOption) {
                      hoverHTML +=
                        '<b class="onPageQuiz-default">' +
                        (j + 1) +
                        '.</b> <span class="answer_correct_' +
                        onPageQuizID +
                        ` onPageQuiz-default">
                    <span id="correct-` +
                        j.toString() +
                        ...
                        options[j] +
                        '</span>' +
                        ' <img id="answer_sym_" src="' +
                        this.custom_icon[9] +
                        '" style="height:25px;display: none;position:absolute;right:18px;margin-top:-23px;"/></span><br/>';
                    } else {
                      hoverHTML +=
                        '<b class="onPageQuiz-default">' +
                        (j + 1) +
                        '.</b> <span class="answer_wrong_' +
                        onPageQuizID +
                        ' onPageQuiz-default"><span id="wrong-' +
                        j.toString() +
                        '" name="quiz" value="' +
                        options[j] +
                        '">' +
                        ...
                        ' <img id="answer_sym_" src="' +
                        this.custom_icon[10] +
                        '" style="height:20px;margin-top:-20px;display: none;position:absolute;right:20px;"/></span><br/>';
                    }
                  }
                  onPageQuizID++;
                  hoverHTML += '<span class="tag-learnt-marked">Learnt</span>';
                  hoverHTML += '</div>';
                  translatedWords[i].innerHTML =
          ...
          if (!buttonList[i].classList.contains('done')) {
            buttonList[i].addEventListener('click', function() {
              ...
              chrome.storage.local.get(['learntWords'], function(result) {
                chrome.storage.local.get(['activityProgressStore'], function(
                  res
                ) {
                  let date =
                    new Date().getDate() +
                    ...
                    '.' +
                    new Date().getFullYear();
                  res.activityProgressStore[date]['words-learnt'] += 1;
                  chrome.storage.local.set({
                    activityProgressStore: res.activityProgressStore,
                  });
                });
                ...
                if (!exist) {
                  if (learntWords.length === 2) {
                    ...
                    updatedLearntWords =
                      updatedLearntWords.split(')')[0] + '|' + wordLearnt + ')';
                  }

                  var snackBar = document.createElement('div');
                  snackBar.setAttribute(
                    'style',
                    'left: 0px;right: 0px;margin-left:auto;margin-right:auto;max-width:40%;background-color:#4ea03b;z-index:10000;border-radius:7.5px;color:white;bottom: 5%;position:fixed;padding:15px;text-align:center;box-shadow: 0 1.5px 4px rgba(0, 0, 0, 0.24), 0 1.5px 6px rgba(0, 0, 0, 0.12);'
                  );
                  var snackContent = document.createElement('p');
                  snackContent.setAttribute(
                    'style',
                  ...
                  undo.setAttribute(
                    'style',
                    'font-weight:bold;cursor: pointer;margin-top:0px;margin-bottom:0px;'
                  );
          ...
          if (!buttonList[i].classList.contains('done')) {
            buttonList[i].addEventListener('click', function() {
              ...
              chrome.storage.local.get(['savedTranslations'], function(result) {
                let updatedSavedTranslations = JSON.parse(
                  result.savedTranslations
                );
                chrome.storage.local.get(['activityProgressStore'], function(
                  res
                ) {
                  let date =
                    ...
                    '.' +
                    new Date().getFullYear();
                  res.activityProgressStore[date]['saved-translation'] += 1;
                  chrome.storage.local.set({
                    activityProgressStore: res.activityProgressStore,
                  });
                });
                ...
                if (!exist) {
                  ...
                  snackBar.setAttribute(
                    'style',
                    'left: 0px;right: 0px;margin-left:auto;margin-right:auto;max-width:40%;background-color:#f19b42;z-index:10000;border-radius:7.5px;color:white;bottom: 5%;position:fixed;padding:15px;text-align:center;box-shadow: 0 1.5px 4px rgba(0, 0, 0, 0.24), 0 1.5px 6px rgba(0, 0, 0, 0.12);'
                  );
                  var snackContent = document.createElement('p');
                  snackContent.setAttribute(
                    'style',
                  ...
                  undo.addEventListener('click', function() {
                    //Undo the change
                    var toUndo = undo.getAttribute('snack_original');
                    chrome.storage.local.get(['savedTranslations'], function(
                      re
                    ) {
                      ...
                      chrome.storage.local.set({
                        savedTranslations: JSON.stringify(arrChange),
                      });
                    });
                  });
                  chrome.storage.local.set({
                    savedTranslations: JSON.stringify(updatedSavedTranslations),
                  });
                  snackBar.appendChild(snackContent);
                  snackBar.appendChild(undo);
        ...
        for (let i = 0; i < buttonList.length; i++) {
          if (!buttonList[i].classList.contains('done')) {
            buttonList[i].addEventListener('click', function() {
              ...
              chrome.storage.local.get('userBlacklistedWords', function(
                result
              ) {
                var currentUserBlacklistedWords = result.userBlacklistedWords;
                var blacklistedWords = [];
                blacklistedWords = currentUserBlacklistedWords
                  .slice(1, -1)
                  .split('|');
                ...
                if (blacklistedWords.indexOf(wordToBeBlacklisted) === -1) {
                  ...
                  if (!currentUserBlacklistedWords) {
                    ...
                    updatedBlacklistedWords =
                      currentUserBlacklistedWords.split(')')[0] +
                      '|' +
                      wordToBeBlacklisted +
                      ')';
                  }
                ...
                snackBar.setAttribute(
                  'style',
                  'left: 0px;right: 0px;margin-left:auto;margin-right:auto;max-width:40%;background-color:#d5446a;z-index:10000;border-radius:7.5px;color:white;bottom: 5%;position:fixed;padding:15px;text-align:center;box-shadow: 0 1.5px 4px rgba(0, 0, 0, 0.24), 0 1.5px 6px rgba(0, 0, 0, 0.12);'
                );
                var snackContent = document.createElement('p');
                snackContent.setAttribute(
                  'style',
                ...
                snackContent.innerText =
                  "'" + wordToBeBlacklisted + "' blacklisted";
                ...
                undo.addEventListener('click', function() {
                  ...
                  chrome.storage.local.get(['userBlacklistedWords'], function(
                    ...
                    chrome.storage.local.set({
                      userBlacklistedWords: arrChange,
                    });
                  });
                });
        ...
        for (let i = 0; i < buttonList.length; i++) {
          if (!buttonList[i].classList.contains('done')) {
            buttonList[i].addEventListener('click', function() {
              ...
              var searchUrl =
                'http://' + targetLanguage + '.wiktionary.org/wiki/' + word;
        ...
        for (let i = 0; i < buttonList.length; i++) {
          if (!buttonList[i].classList.contains('done')) {
            buttonList[i].addEventListener('click', function() {
              var word = this.getAttribute('data-translated');
              var searchUrl =
                'http://www.google.com/search?lr=lang_' +
                targetLanguage +
                '&q=' +
                word +
                '&tbm=isch';
        ...
        for (let j = 0; j < buttonList.length; j++) {
          if (!buttonList[j].classList.contains('done')) {
            buttonList[j].addEventListener('click', function() {
              ...
              this.style =
                'background-color: #55BB38; color: white; border-radius: 2px; padding: 2px 4px 2px 4px; font-weight: bold';
            });
        ...
        for (let j = 0; j < buttonList.length; j++) {
          if (!buttonList[j].classList.contains('done')) {
            buttonList[j].addEventListener('click', function() {
              ...
              this.style =
                'background-color: #D20026; color: white; border-radius: 2px; padding: 2px 4px 2px 4px; font-weight: bold';
            });
            buttonList[j].className += ' done';
          }
        }
                ...
                result.then(res => {
                  ...
                  if (dicEntries.length) {
                    ...
                    dicEntries.forEach(obj => {
                      let count = 0,
                        tranlations = obj.tr;
                      details += '<b>Type: ' + obj.pos + '</b><br/>Use Case:';
                      tranlations.forEach(tr => {
                        ...
                        if (tr.syn) {
                          details +=
                            '<br/><i class="hc-info-details-syn"> <b>Synonyms:</b> ';
                          let synAr = tr.syn,
                            k = 0;
                          synAr.forEach(syn => {
                            k++;
                            details += syn.text;
                            if (k !== synAr.length) details += ', ';
                          });
                          details += '</i>';
                        }
                      });
                    ...
                    let details =
                      '<hr/><span class="hc-info-details-not"><b>Details Not Available</b></span>';
                    parentElement.innerHTML += details;
                  }
                });
              } else {
                      ...
                      if (dicEntries.length) {
                        ...
                        dicEntries.forEach(obj => {
                          ...
                          tranlations.forEach(tr => {
                            ...
                            if (tr.syn) {
                              details +=
                                '<br/><i class="hc-info-details-syn"> <b>' +
                                trans['Synonyms'] +
                                ':</b> ';
                              let synAr = tr.syn,
                                k = 0;
                        ...
                        let details =
                          '<hr/><span class="hc-info-details-not"><b>' +
                          trans['Details'] +
                          ' ' +
                          trans['Not'] +
                          ' ' +
    ...
    for (let i in paragraph.childNodes) {
      if (paragraph.childNodes[i].nodeType === 3) {
        if (!/^\s*$/.test(paragraph.childNodes[i].textContent)) {
          if (
            this.srcLang === yandexLanguages.Chinese ||
            this.srcLang === azureLanguages['Chinese Simplified'] ||
            this.srcLang === azureLanguages['Chinese Traditional'] ||
            this.srcLang === yandexLanguages.Japanese ||
            this.srcLang === googleLanguages['Chinese Simplified'] ||
            this.srcLang === googleLanguages['Chinese Traditional']
          ) {
            ...
            for (let j in sentences) {
              ...
              for (let k in words) {
                ...
                if (filteredTMap[words[k]]) {
                  let x = sentences[j].replace(
                    words[k],
                    ' ' +
                      iMap[filteredTMap[words[k]]] +
                      '<sup class="countPage">(' +
                      this.countedWords[words[k]] +
                      ')</sup>' +
                      ' '
                  );
                  sentences[j] = x;
            ...
            for (let j in sentences) {
              let words = sentences[j].split(' ');
              var shuffleIndices = _.shuffle(
                Array.apply(null, { length: words.length }).map(
                  Function.call,
                  Number
                )
              );
              for (let k in words) {
                ...
                if (filteredTMap[words[shuffleIndices[k]]]) {
                  words[shuffleIndices[k]] =
                    iMap[filteredTMap[words[shuffleIndices[k]]]] +
                    '<sup class="countPage">(' +
                    this.countedWords[words[shuffleIndices[k]]] +
                    ')</sup>';
                  break;
                }
              }
      ...
      if (paragraph.childNodes[i].nodeType === 3) {
        if (!/^\s*$/.test(paragraph.childNodes[i].textContent)) {
          if (
            ...
            let toBeTranslated = Math.floor(
              (words.length * this.translationProbability) / 100
            );
            let actualCount = 0;
            for (let k in words) {
  ...
  toggleAllElements() {
    ...
    for (var i = 0; i < words.length; i++) {
      ...
      if (isNaN(word.innerText)) {
        ...
        word.innerText = this.translated
          ? word.dataset.translated
  ...
  clkTest(str) {
    ...
    return (
      clk_main.test(str) ||
      clk_extension.test(str) ||
      clk_strokes.test(str) ||
      clk_symbols_punctuation.test(str)
    );
  }
...
chrome.storage.local.get(null, res => {
  ...
  if (
    res.activation === true &&
    !document.URL.startsWith('chrome-extension') &&
    ...
    if (blacklistWebsiteReg.test(document.URL) && res.blacklist !== '()') {
      ...
      (res.srcLang === '' || res.targetLanguage === '') &&
      res.userDefinedOnly === false
    ) {
      console.log(
        '[MTW] No active pattern. Please select a pattern in the options page.'
      );
    } else {
      if (res.useCommonWordsOnly === true) {
        console.log('[MTW] Using Common Words for Translation Only.');
      }
      MTWTranslator.initialize(res, false);
      window.onload = () => {
        function sleep(ms) {
            ...
            List.forEach(node => {
              try {
                ...
                if (
                  ...
                  len &&
                  allow &&
                  MTWTranslator.finishTranslation &&
                  !e.contains('mtwProcessed')
                ) {
                  scanCount++;
                  if (scanCount >= previousParagraphs.length / 20) {
                    ...
                    if (MTWTranslator.finishTranslation) {
                      ...
                      !MTWTranslator.finishTranslation ||
                      scanCount >= previousParagraphs.length / 20
                    ) {
                      scanCount = 0;
                    }
                  }
  ...
  if (status === 'success') {
    ...
    if (status === 'fail') {
      timer = setInterval(function() {
        ...
        mtwReconnectTime.innerHTML =
          'Could not connect to Translator Service Reconnecting in ' +
          tempTime +
          's  &nbsp;....&nbsp;';
    ...
    var styleConnectionHead =
      'position: fixed; top:0;width: 100%; display: flex; justify-content: center; align-items: center; background: red; padding: 0.5em 0; color: white !important; font-size: 0.9em; z-index: 1000;';
    mtwConnectionHead.setAttribute('id', 'mtw-connection-head');
    mtwConnectionHead.setAttribute('style', styleConnectionHead);
    mtwConnectionHead.innerHTML =
      '<div><strong>MTW:</strong>  <span id="mtw-reconnect-time">Could not connect to Translator Service Reconnecting in ' +
      tempTime +
      's  &nbsp;....&nbsp;</span><span id="mtw-reconnect-now"  style="cursor:pointer;"><strong>Reconnect Now</strong></span> </div> <span id="mtw-connection-cross" style="position: absolute; right: 1em; font-size: 1.2em;cursor:pointer;">✕</span> </div>';
    if (status === 'fail')
