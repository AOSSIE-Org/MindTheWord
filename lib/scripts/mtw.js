...
export class ContentScript {
  ...
  processTranslations(translationMap, userDefinedTMap, cwMatch) {
   ...
   var wordFrequencyPromise = new Promise((resolve, reject) => {
    ...
    chrome.storage.local.get(['wordFrequencyCorpus','totalWordsperLanguage'], (wordObjectList) => {
      ...
      //2--6--11--2018,博客,1,博客,1,一个,1,博客,1,一个,1,修订,1,博客,1,一个,1,修订,1,网页,1,博客,1,一个,1,修订,1,网页,1,发布,1,博客,1,一个,1,修订,1,网页,1,发布,1,为什么,1,博客,1,一个,1,修订,1,网页,1,发布,1,为什么,1,做,1,博客,1,一个,1,博客,1,一个,1,修订,1,博客,1,一个,1,修订,1,网页,1,博客,1,一个,1,修订,1,网页,1,发布,1,为什么,1,做,1,需要,1,博客,1,一个,1,修订,1,网页,1,发布,1,为什么,1,做,1,需要,1,测试,1,博客,1,一个,1,修订,1,网页,1,发布,1,为什么,1,做,1,需要,1,测试,1,要,1,博客,1,一个,1,修订,1,网页,1,发布,1,为什么,1,做,1,需要,1,测试,1,要,1,确保,1,博客,1,一个,1,修订,1,网页,1,发布,1,为什么,1,做,1,需要,1,测试,1,要,1,确保,1,我们,1,博客,1,一个,1,修订,1,网页,1,发布,1,为什么,1,做,1,需要,1,测试,1,要,1,确保,1,我们,1,程序
          ...
          if (!(wordsArray.includes(word))) { // if word is not learnt
            ...
            for(let j=0;j< options.length; j++) {
              if (j === correctAnsOption) {
                hoverHTML += `<span class="answer_`+ onPageQuizID +`"><b>`+ (j+1) +`. </b> <input type="checkbox" id="correct-`+ j.toString()
                  +'" name="quiz" value="'+ options[j] +'" />'+ options[j]
                  +' <img id="answer_sym_" src="'+ this.custom_icon[9]  
                  + '" style="height:25px;display:none;position:absolute;right:18px;margin-top:-20px;"/></span><br/>' ;
              }
              else {
                hoverHTML += `<span class="answer_`+ onPageQuizID +`"><b>`+ (j+1) 
                  +`. </b> <input type="checkbox" id="wrong-`+ j.toString() +'" name="quiz" value="'+ options[j] +'" />'+ options[j]
                  +' <img id="answer_sym_" src="'+ this.custom_icon[10]  
  ...
  postProcessing(){
    ...
    for (let i = 0; i < paragraphs.length; i++) {
      ...
      buttonList = paragraphs[i].querySelectorAll("[class^=answer_]"); 
      if (buttonList.length) {
        for (let j = 0; j < buttonList.length; j++) {
          if(!buttonList[j].classList.contains('done')){
            buttonList[j].addEventListener('click', function() {
              ...
              this.innerHTML = this.innerHTML.replace('type="checkbox"', 'type="checkbox" checked=true ');
            });
