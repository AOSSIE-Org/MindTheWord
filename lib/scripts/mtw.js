...
export class ContentScript {
  ...
  initialize(res, isTSExample) {
    ...
    if (!isTSExample) {
      ...
      console.log('ele is ');
      console.log(ele);
      this.allparagraphs = document.getElementById('ts-example').querySelectorAll('p');
      console.warn(this.allparagraphs);
    }
  ...
  filterParagraphsQuizzing() {
    chrome.storage.local.get('quizQuestionsStore', res => {
      ...
      quizSample.forEach(domNode => {
        ...
        } else if (domSize >= 200 && domSize < 500) { // skip large size paragraphs
          let inst = {
            'type': 'paragraph',
            'text': domNode.innerText.trim(),
            'shown': 0,
  ...
  activityProgress(translations) {
    ...
    if (prev === undefined) {
      ...
      for (let keys in this.activityProgressStore) {
        max = this.activityProgressStore[keys]['day-number'] > max ? this.activityProgressStore[keys]['day-number'] : max;
      }
    ...
    let visited = this.activityProgressStore[date]['websites-visited'], found = false;
    ...
    for (let webObjects in this.activityProgressStore[date]['websites-visited']) {
      if (this.activityProgressStore[date]['websites-visited'][webObjects]['url'] === URL) {
        for (let keys in translations) {
          this.activityProgressStore[date]['websites-visited'][webObjects]['tMap'][keys] = translations[keys];
        }
        this.activityProgressStore[date]['websites-visited'][webObjects]['translation-count'] = Object.keys(this.activityProgressStore[date]['websites-visited'][webObjects]['tMap']).length;
        break;
      }
    }
    chrome.storage.local.set({ 'activityProgressStore': this.activityProgressStore });
  }
  ...
  processTranslations(translationMap, userDefinedTMap, cwMatch) {
    console.warn('insideeeeeeeeeeee');
    var filteredTMap = {};
    ...
    //for quiz feature
    chrome.storage.local.set({ 'translatedWordsForQuiz': JSON.stringify(this.translatedWordsForQuiz) });
    ...
    var wordFrequencyPromise = new Promise((resolve, reject) => {
      ...
      console.warn('allTranslated is ');
      console.warn(allTranslated);
        ...
        for (let a = 0; a < paragraphs.length; a++) {
          ...
          for (let i = 0; i < translatedWords.length; i++) {
            ...
            if (!(wordsArray.includes(word))) { // if word is not learnt
              console.warn('assigning hovercards');
              ...
              hoverHTML += '<img style="cursor:pointer;" class="mtwSpeak" title="Speak" data-translated="' + translatedWords[i].getAttribute('data-translated') + '" src="' + this.custom_icon[0] + '">';
              if (this.detailsApiKey && this.optionalAPI) {
                hoverHTML += '<img style="cursor:pointer;" class="moreInformation" title="More Information" data-original="' + translatedWords[i].getAttribute('data-original') + '" src="'+ this.custom_icon[11] +'">';
              }
              hoverHTML += '<img style="cursor:pointer;" class="mtwMarkAsLearnt mtwHovercardBelow" title="Mark As Learnt" data-original="' + translatedWords[i].getAttribute('data-original') + '" src="' + this.custom_icon[1] + '">';
              hoverHTML += '<img style="cursor:pointer;" class="mtwSaveTranslation mtwHovercardBelow" title="Save Translation" data-translated="' + translatedWords[i].getAttribute('data-translated') + '" data-original="' + translatedWords[i].getAttribute('data-original') + '" src="' + this.custom_icon[4] + '">';
              hoverHTML += '<img style="cursor:pointer;" class="mtwBlacklistWord mtwHovercardBelow" title="Blacklist Word" data-original="' + translatedWords[i].getAttribute('data-original') + '" src="' + this.custom_icon[5] + '">';
              hoverHTML += '<img style="cursor:pointer;" class="mtwWordInfo mtwHovercardBelow" title="Word Info" data-translated="' + translatedWords[i].getAttribute('data-translated') + '" src="' + this.custom_icon[3] + '">';
              hoverHTML += '<img style="cursor:pointer;" class="mtwVisualHint mtwHovercardBelow" title="Visual Hint" data-translated="' + translatedWords[i].getAttribute('data-translated') + '" src="' + this.custom_icon[2] + '">';
              console.warn('the loop out');
              console.warn(translatedWords[i]);
          ...
          if(!buttonList[i].classList.contains('done')){
            buttonList[i].addEventListener('click', function() {
              ...
              chrome.storage.local.get(['learntWords'], function(result) {
                chrome.storage.local.get(['activityProgressStore'], function(res) {
                  let date = new Date().getDate() + '.' + new Date().getMonth() + '.' + new Date().getFullYear();
                  res.activityProgressStore[date]['words-learnt'] += 1;
                  chrome.storage.local.set({ 'activityProgressStore': res.activityProgressStore });
                });
...
chrome.storage.local.get(null, (res) => {
  ...
  if (res.activation === true && !document.URL.startsWith('chrome-extension')) {
