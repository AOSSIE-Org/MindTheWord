...
export class ContentScript {
  ...
  processTranslations(translationMap, userDefinedTMap, cwMatch) {
    ...
    //adding language specific message on hover card
    var translateHoverCardWords = new Promise((resolve, reject) => {
      this.hoverWords = {Occurs : 2,times : 3,till : 1,Used : 1,current : 1,view : 1,rarely : 1,
        approximately : 1,in : 1,every : 2,1000 : 1,words : 1,100:1};
      let translateHoverObject = {};
      switch (this.translator) {
        case 'Yandex':
        translateHoverObject = new YandexTranslate(this.yandexTranslatorApiKey, 'en', this.srcLang);
          break;
        case 'Bing':
        translateHoverObject = new BingTranslate(this.bingTranslatorApiKey, 'en', this.srcLang);
          break;
        case 'Google':
        translateHoverObject = new GoogleTranslate(this.googleTranslatorApiKey, 'en', this.srcLang);
          break;
        default:
          console.error('No such translator supported');
      }
      translateHoverObject.getTranslations(this.hoverWords).then((tMap_hover) => {
        resolve(tMap_hover);
      });
    });
    translateHoverCardWords.then((hoverTranslations) => {
      this.hoverTranslations = hoverTranslations;
        ...
        chrome.storage.local.get(['wordFrequencyCorpus','totalWordsperLanguage'], (wordObjectList) => {
          ...
          for(var i=0; i< allTranslated.length; i++) {
            ...
            if(found===false) {
              ...
              wordObject.srclang = this.targetLanguage;
              wordObject.word = word;
              nativeWords.push(wordObject);
            }
          }
          // calculating total words translated
          let foundLang = false;
          for(var x=0;x< totalWordsLanguage.length; x++ ){
            if(totalWordsLanguage[x].srcLang===this.srcLang) {
              foundLang = true;
          ...
          if(foundLang==false){
            totalWordsLanguage.push({srclang:this.srcLang,count:allTranslated.length});
      ...
      wordFrequencyPromise.then((nativeWords) => {
        ...
        for (let i = 0; i < paragraphs.length; i++) {
          ...
          for (let i = 0; i < translatedWords.length; i++) {
            ...
            var word = translatedWords[i].getAttribute('data-original');
            for(var key in this.countedWords){
              if(this.countedWords.hasOwnProperty(key)){
                if(key === word){
                  hoverHTML += '<ul><li> '+s.Used+' '+ this.countedWords[key] +' '+s.times+' '+ s.till+' '+ s.current+' '+ s.view+' </li>';
                  break;
                }
              }
            }
            for(var k =0; k< nativeWords.length; k++) {
              if (nativeWords[k].word===translatedWords[i].getAttribute('data-translated')){
                // if the user wants the content in hover card to be in english
                if(this.srcLang == 'en') { 
                  hoverHTML += '<li> '+s.Occurs+' '+
                  (Math.round(( nativeWords[k].count / this.totalWordsLanguage ) * 100) == 0 
                    ?  ((Math.round(( nativeWords[k].count / this.totalWordsLanguage )*100) * 1000 == 0) ? s.rarely + ' </li></ul>' :
                    s.approximately+ ' '+Math.round(( nativeWords[k].count / this.totalWordsLanguage )*10) * 1000+' '+s.times+' '+ s.in+' '+
                      s.every+' 1000 '+ s.words+' </li></ul>')
                    :s.approximately+ ' '+ Math.round(( nativeWords[k].count / this.totalWordsLanguage ) * 100)+' '+ s.times+' '+
                    s.in+' '+ s.every+' '+ ' 100 '+ s.words+' </li></ul>'); // prevent showing 0 in cases of ratios of < 0.5
                  break;
                }
                else { 
                  var first = ((Math.round(( nativeWords[k].count / this.totalWordsLanguage )*100) == 0) // implies rare events
                    ? '<li> '+s.every+' 1000 '+ s.words+' '+s.in+' ' 
                    : '<li> '+s.every+' 100 '+ s.words+' '+s.in+' ');

                  var second = (Math.round(( nativeWords[k].count / this.totalWordsLanguage ) * 100) == 0 )
                    ?  ((Math.round(( nativeWords[k].count / this.totalWordsLanguage )*100) * 1000 == 0) 
                          ? s.rarely + ' </li></ul>' 
                          : s.approximately+ ' '+Math.round(( nativeWords[k].count / this.totalWordsLanguage )*10) * 1000+' '+s.times+' </li></ul>')
                    :  s.approximately+ ' '+ Math.round(( nativeWords[k].count / this.totalWordsLanguage ) * 100)+' '+ s.times
                          +' </li></ul>'; // prevent showing 0 in cases of ratios of < 0.5
                  hoverHTML += first + second;
                  break;
                }
