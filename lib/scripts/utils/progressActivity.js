/**
 * ProgressActivity is a handler class that controls the entire activity
 * in the Home screen and assigns the charts with the help of D3.js
 */
...
export class ProgressActivity {
  constructor(parentElementID, svgWidth, svgHeight, outerElementWidth) {
    ...
    this.blockedWebsitesContainer = document.getElementById(
      'ap-blocked-websites'
    );
    this.setSecondaryContainers('ap-blocked-websites', 'ap-blocked-words');
    this.lastElementID = null;
    this.colorShades = ['#EBEDF0', '#C6E48B', '#7BC96F', '#239A3B', '#196127'];
  }

  /**
   * Returns the day based on the index.
   * @param {number} n - day index
   * @returns {String} - respective day
   */
  getDay(n) {
    switch (n) {
      case 0:
  ...
  /**
   * Return total translations done.
   * @param {number} tMaplist - list of translation numbers of each day
   */
  ...
  /**
   * Sort the guage object based on date.
   * @param {Object} object - object of data points on the respective date in unsorted format
   * @returns {Object} object - object of data points on the respective date in sorted format
   */
  sort(object) {
    ...
    this.lastElementID =
      ((list.length - 1) * this.xAxisUnit).toString() + '_node_id';
    return object;
  }

  /**
   * Algorithm that assigns color to individual bubbles based on the usage.
   * @param {Object} dataPoints - object of datapoints on each date
   * @returns {String} colors - array of the corresponding color codes
   */
  ...
  /**
   * Create structured datapoints from unstructured ones.
   * @param {Object} rawPoints - unstructured datapoints as raw
   * @returns {Promise}
   */
  ...
  /**
   * Set scale on the Y-axis.
   * @param {Object} rawPoints - unstructured datapoints as raw
   * @returns {Object} for number of divisions in X-axis and words covered in each change in Y-axis unit
   */
  setScalingY(dataPoints) {
    let max = 0;
    for (let point in dataPoints) {
      max =
        parseInt(dataPoints[point]['words-translated']) > max
          ? parseInt(dataPoints[point]['words-translated'])
          : max;
    }
  ...
  /**
   * Generate bubbles in the activity screen
   * @param {number} coordinatesList - list of coordinates
   * @param {number} yScaleDiv - divisions on Y-axis
   */
  generateCircles(coordinatesList, yScaleDiv) {
    let circles = this.container
      .selectAll('circle')
  ...
  /**
   * Generate curve in the activity screen
   * @param {number} coordinatesList - list of coordinates
   * @param {number} yScaleDiv - divisions on Y-axis
   */
  generateCurves(coordinatesList, yScaleDiv) {
    ...
    for (let inst in coordinatesList) {
      coordinatesList[inst].y_axis =
        this.svgHeight - coordinatesList[inst].y_axis / yScaleDiv;
      coordinatesList[inst].x_axis =
        this.xAxisUnit * coordinatesList[inst].x_axis + 15;
    }

    this.container
      .append('path')
      .attr('d', line(coordinatesList))
      .attr('stroke', 'black')
      .attr('stroke-width', '2')
      .attr('fill', 'none');
  }
  ...
  /**
   * Generate and assign hovercards in the activity screen
   * @param {number} coordinatesList - list of coordinates
   * @param {number} yScaleDiv - divisions on Y-axis
   * @param {Object} yScaleDiv - structured data as guage points
   */
    ...
    for (let i = 0; i < size; i++) {
      let element = document.createElement('div'),
        parentElement = document.getElementById(
          i * this.xAxisUnit + '_node_id'
        );
      element.id =
        coordinatesList[i].x_axis * this.xAxisUnit + '_node_id_hovercard';
      element.innerHTML =
        'words-translated: <b class="key-styles">' +
        data[i]['words-translated'] +
        `</b> <br>
        words-learnt: <b class="key-styles">` +
        data[i]['words-learnt'] +
        `</b> <br>
        saved-translation: <b class="key-styles">` +
        data[i]['saved-translation'] +
        `</b> <br>
        websites-visited: <b class="key-styles">` +
        data[i]['websites-visited'] +
        '</b>';
      ...
      parentElement.onmousemove = function(eve) {
        ...
        ele.onclick = function() {
          ...
          document.getElementById('ap-websites-visited').innerHTML =
            '<div class="ap-websites-head"> Websites Visited</div>';
          chrome.storage.local.get('activityProgressStore', res => {
            ...
            for (let url in resp[date]['websites-visited']) {
              ...
              urlLocal =
                urlLocal.length > 30
                  ? urlLocal.substring(0, 27) +
                    ' ... ' +
                    urlLocal.substring(urlLocal.length - 6)
                  : urlLocal;
              element.innerHTML = ++count + '. ' + urlLocal + ' <br/>';
              element.className = 'ap-list-urls';
              element.setAttribute(
                'url',
                resp[date]['websites-visited'][url]['url']
              );
              element.setAttribute('date', date);
              element.onclick = function() {
                let url = this.getAttribute('url'),
                  date = this.getAttribute('date');
                document.getElementById('ap-websites-tMap').innerHTML =
                  '<div class="ap-websites-head">Traslation Map</div>';
                chrome.storage.local.get('activityProgressStore', res => {
                  let resp = res.activityProgressStore;
                  ...
                  if (location !== -1) {
                    ...
                    for (let keys in tMap) {
                      ...
                      document
                        .getElementById('ap-websites-tMap')
              ...
              document
                .getElementById('ap-websites-visited')
                .appendChild(element);
            }
          });
        };
  ...
  /**
   * Create axes
   * @param {Number} upperBoundY - upper bound value of Y-axis
   * @param {Number} count - number of structured datapoints
  ...
  drawAxis(upperBoundY, count, div) {
    ...
    labelHead.style.top = document
      .getElementById('axis-x-' + 0)
      .getBoundingClientRect().top;
    labelHead.style.left =
      document.getElementById('axis-x-' + 0).getBoundingClientRect().left + 10;
    ...
    for (let i = 1; i <= count; i++) {
      ...
      label.style.top = document
        .getElementById('axis-x-' + i)
        .getBoundingClientRect().top;
      label.style.left =
        document.getElementById('axis-x-' + i).getBoundingClientRect().left +
        10;
    ...
    for (let j = 0; j < 6000; j += 100) {
      // initialising chart for the month
      let i = j + 15;
      this.container
        .append('line')
        .attr('x1', i)
  ...
  /**
   * Set values for other sections like blocked websites and blocked words in the activity screen.
   * @param {String} idBlockedWebsites - id of the container that displays the blocked websites
   * @param {String} idBlockedWords - id of the container that displays the blocked words
  ...
  /**
   * Controller for handling creation of interactive and dynamic graphs.
   * @param {Object} rawPoints - struc
   */
  generate(rawPoints) {
    if (document.getElementById(this.parentElementID.substring(1)) !== null) {
      if (rawPoints !== null && Object.keys(rawPoints).length !== 0) {
        this.setDataPoints(rawPoints).then(res => {
          ...
          this.generateHovercards(
            coordinatesList,
            scaleDiv.wordsPerUnitYScale,
            this.dataPoints
          );
        ...
        let element = document.createElement('div');
        element.innerHTML =
          'No Translation Data Found! Please use the extension to get the activity results.';
        ...
        throw new Error(
          'Unable to generate activity progress graph. Data-Points not found'
        );
      }
    } else {
      setTimeout(() => {
        this.generate(rawPoints);
      }, 1000);
    }
