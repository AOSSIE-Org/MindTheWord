...
export class QuizzingSentence {
  // servers as a base class for other quizzing types as well
  constructor(
    parentElementID,
    srcLang,
    targetLang,
    ...
    chrome.storage.local.get(['quizQuestionsStore', 'anki'], resStore => {
      ...
      if (Object.keys(res).length === 4) {
        document.getElementById('quiz-sentence-question').innerHTML =
          'No data found. Please use the extension in order to practice the quiz.';
        document.getElementById('writingQuizLoader').style.display = 'none';
      } else {
        let min = 10000,
          minID = '';
        for (let keys in res) {
        ...
        try {
          if (quizCount / 5 === 1) {
            originalText =
              Anki['quiz-sentence']['medium'][
                Object.keys(Anki['quiz-sentence']['medium'])[0]
              ];
            minID = Object.keys(Anki['quiz-sentence']['medium'])[0];
            quizCount = -1;
          } else if (quizCount / 2 === 1) {
            originalText =
              Anki['quiz-sentence']['hard'][
                Object.keys(Anki['quiz-sentence']['hard'])[0]
              ];
            minID = Object.keys(Anki['quiz-sentence']['hard'])[0];
          } else {
            originalText = generateQuestionsMatching(
              toList(res),
              Anki,
              'sentence'
            );
        ...
        this.getTMaps(processedText).then((tMap, rej) => {
          ...
          for (let originalWord in tMap) {
            ...
            originalText = originalText.replace(
              ' ' + originalWord + ' ',
              ' <span class="quiz-sentence-questions-class" translated-word="' +
                tMap[originalWord] +
                '" original-word="' +
                originalWord +
                '"></span> ' +
            ...
            if (c === Object.keys(tMap).length) {
              ...
              doneButton.onclick = () => {
                ...
                document
                  .getElementById(self.parentElementID)
                  .appendChild(SelectRatingAnki);
                let nextButton = document.createElement('button');
                nextButton.textContent = 'Next';
                nextButton.className = 'btn btn-success';
                nextButton.onclick = () => {
                  let ankiLevel = document.getElementById(
                    'anki-quiz-sentence-rating'
                  ).value;
                  // delete old records
                ...
                let optionDOMsParent = document.getElementsByClassName(
                    'quiz-sentence-questions-class'
                  ),
                  optionsDOM = document.getElementsByClassName(
                    'quiz-sentence-question-options'
                  );
                for (
                  let i = 0, j = 0;
                  i < optionDOMsParent.length && j < optionsDOM.length;
                  i++, j++
                ) {
                  ...
                  if (translatedWord === elementValue) {
                    document.getElementById(
                      'quiz-' + element.getAttribute('original-word') + '-tick'
                    ).style.display = 'inline';
                  } else {
                    document.getElementById(
                      'quiz-' + element.getAttribute('original-word') + '-cross'
                    ).style.display = 'inline';
                  }
                }
              };
...
export class QuizzingOral extends QuizzingSentence {
  ...
  setQuestions() {
    chrome.storage.local.get(['anki', 'translatedWordsForQuiz'], res => {
      ...
      document.getElementById('quiz-oral-question-translation').innerHTML =
        'Translated Word: ' + answer;
      ...
      nextButton.onclick = () => {
        ...
        chrome.storage.local.get('anki', res => {
          let Anki = JSON.parse(res.anki),
            answerQuiz = document
              .getElementById('quiz-oral-question')
              .getAttribute('answer'),
          ...
          document
            .getElementById('quiz-oral-area')
            .removeChild(SelectRatingAnki);
    ...
    if (this.count % 2 !== 0) {
      ...
      this.speech.onresult = function(eve) {
        if (eve.results.length > 0) {
          // marks presence of transcripted words
          messageArea.value +=
            eve.results[eve.results.length - 1][0].transcript;
        }
      };
      return true;
    }
...
export class QuizzingAural extends QuizzingSentence {
  ...
  setOptionsWords(q, len, exceptIndex, question) {
    ...
    for (let i = 0; i < optionsArr.length; i++) {
      ...
      if (i === correctAnswerIndex) {
        option.onclick = () => {
          document.getElementById('__ans_img_' + optionsArr[i]).style.display =
            'inline';
...
function generateQuestionsMatching(questions, ankiStore, code) {
  ...
  switch (code) {
    ...
    case 'aural':
      ...
      if (
        Object.keys(anki['medium']).length > Object.keys(anki['hard']).length
      ) {
        let list = toList(questions);
        try {
          if (Object.keys(anki['medium'])[0].length !== '') {
            let question = findBestMatch(
              Object.keys(anki['medium'])[0],
              filterTarget(Object.keys(anki['medium'])[0], list)
            ).bestMatch.target;
            let answer = questions[question];
            delete questions[question];
            chrome.storage.local.set({
              translatedWordsForQuiz: JSON.stringify(questions),
            });
            return {
              question: question,
              answer: answer,
        ...
        try {
          if (Object.keys(anki['hard'])[0].length !== '') {
            let question = findBestMatch(
              Object.keys(anki['hard'])[0],
              filterTarget(Object.keys(anki['hard'])[0], list)
            ).bestMatch.target;
            let answer = questions[question];
            delete questions[question];
            chrome.storage.local.set({
        ...
        try {
          if (Object.keys(anki['medium'])[0].length !== '') {
            return findBestMatch(
              ...
              filterTarget(anki['medium'][Object.keys(anki['medium'])[0]], list)
            ).bestMatch.target;
          } else return '';
        } catch (e) {
          return '';
        ...
        try {
          if (Object.keys(anki['hard'])[0].length !== '') {
            return findBestMatch(
              ...
              filterTarget(anki['hard'][Object.keys(anki['hard'])[0]], list)
            ).bestMatch.target;
          } else return '';
        } catch (e) {
          return '';
