...
export class ContextMenu {
  ...
  isWordCountNone(inputString) {
    ...
    if (s.split(' ').length !== 1) {
      alert(
        'Please select a single word only. Phrases such as: "' +
          inputString +
          '" aren\'t allowed'
      );
      return true;
    ...
    chrome.storage.local.get('blacklist', result => {
      ...
      if (blacklistURLs.indexOf(domainNameFromTabURL + '*') === -1) {
        ...
        if (!currentBlacklist) {
          ...
          updatedBlacklist =
            currentBlacklist.split(')')[0] + '|' + domainNameFromTabURL + '*)';
        }
      }
  ...
  whitelistURL(url) {
    chrome.storage.local.get('blacklist', data => {
      ...
      chrome.storage.local.set({
        blacklist: '(' + currentBlacklist.join('|') + ')',
      });
    });
    chrome.tabs.getSelected(null, tab => {
      var code = 'window.location.reload();';
  ...
  searchForSimilarWords(selectedText, searchPlatform) {
    var searchPlatformURLS = {
      bing:
        'http://www.bing.com/search?q=%s+synonyms&go=&qs=n&sk=&sc=8-9&form=QBLH',
      google: 'http://www.google.com/#q=%s+synonyms&fp=1',
      googleImages:
        'http://www.google.com/search?num=10&hl=en&site=imghp&tbm=isch&source=hp&q=%s',
    ...
    chrome.storage.local.get(null, data => {
      ...
      chrome.tts.getVoices(voices => {
        voices.forEach(voice => {
          ...
          if (
            voice.voiceName === playbackOptions.voiceName &&
      ...
      if (words.length >= 2 && words.length < 14) {
        ...
        chrome.storage.local.get(keys, data => {
          ...
          switch (data.translatorService) {
            case 'Yandex':
              let yandexTranslate = new YandexTranslate(
                data.yandexTranslatorApiKey,
                data.sourceLanguage,
                data.targetLanguage
              );
              ...
              break;
            case 'Azure':
              let azureTranslate = new AzureTranslate(
                data.bingTranslatorApiKey,
                data.sourceLanguage,
                data.targetLanguage
              );
        ...
        alert(
          'Select a sentence containing at least 3 words and at most 15 words.'
        );
      }
    });
    return promise;
