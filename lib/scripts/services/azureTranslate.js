      ...
      if (Lists.toString().length >= 4900) {
        ...
        this.detectLanguage(Lists.toString()).then(lang => {
          if (lang === this.srcLang) {
            this.options.body = [{ text: Lists.toString() }];
            http(this.url)
              .post(
                '_bing_' + JSON.stringify(this.options.body),
                this.options.headers
              )
              .then(data => {
                let inJSON = JSON.parse(data);
                let translationsList = inJSON[0]['translations'][0]['text']
                  .substring(-1)
                  .split(',');
    ...
    var promise = new Promise((resolve, reject) => {
      if (text.toString().length >= 4900) {
        ...
        this.detectLanguage(text.toString()).then(lang => {
          if (lang === this.srcLang) {
            this.options.body = [{ text: text.toString() }];
            http(this.url)
              .post(
                '_bing_' + JSON.stringify(this.options.body),
                this.options.headers
